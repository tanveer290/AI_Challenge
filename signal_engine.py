...
def _format_slots(slots, use_hindi_conjunction=False):
    """Format slot strings with '/' or 'ya' depending on language."""
    ...
    for s in slots[:3]:
        if isinstance(s, dict):
            ...
            labels.append(s)
    sep = " ya " if use_hindi_conjunction else " / "
...
def _has_hi_customer(customer):
    """Check if customer prefers Hindi or Hinglish."""
    if not customer:
        return False
    lang = _s(customer.get("language_pref") or (customer.get("preferences") or {}).get("language_pref") or "").lower()
...
def extract_signals(category, merchant, trigger, customer=None):
    ...
    if customer:
        ...
        customer_lang_pref = _s(c_id.get("language_pref") or prefs.get("language"), signals["primary_language"])
        signals["customer_language"]   = customer_lang_pref
        ...
        if trigger_kind in {"recall_due", "customer_lapsed_soft", "customer_lapsed_hard",
            ...
            use_hindi = _has_hi_customer(customer) or ("hi" in customer_lang_pref.lower())
            signals["slots_str"]         = _format_slots(payload.get("available_slots"), use_hindi)
        ...
        last_visit = signals.get("customer_last_visit")
        if last_visit and trigger_kind in {"recall_due", "trial_followup", "winback",
                                            "customer_lapsed_soft", "customer_lapsed_hard"}:
            try:
                lv_date = datetime.fromisoformat(last_visit.replace("Z", "+00:00"))
                delta = datetime.now(lv_date.tzinfo) - lv_date if lv_date.tzinfo else datetime.now() - lv_date
                months = delta.days // 30
                if months > 0:
                    signals["months_since_last_visit"] = months
            except Exception:
                pass
        ...
        if trigger_kind == "trial_followup":
            ...
            signals["slots_str"]        = _format_slots(payload.get("next_session_options"), _has_hi_customer(customer))
    ...
    if trigger_kind == "research_digest":
        ...
        if isinstance(item, dict):
            ...
            signals["research_trial_n_str"]    = f"{_n(item.get('trial_n')):,}" if item.get("trial_n") else ""
        ...
        signals["festival_name"]  = _s(payload.get("match") or payload.get("festival_name") or payload.get("event_name"), "seasonal push")
        ...
        # Attach seasonal beat note if possible
        fest_name = signals["festival_name"].lower()
        for beat in seasonal_beats:
            if isinstance(beat, dict):
                months = _s(beat.get("month_range")).lower()
                if fest_name and fest_name in months:
                    signals["seasonal_beat_note"] = _s(beat.get("note"))
                    break
    ...
    # ── CONTEXT-AWARE BEST OFFER SELECTION ────────────────────────
    # Derive a trigger-specific best offer if applicable
    trigger_offer = None
    if trigger_kind == "bridal_followup":
        for o in active_offers:
            if isinstance(o, dict) and "bridal" in _s(o.get("title")).lower():
                trigger_offer = _s(o.get("title"))
                break
        if not trigger_offer:
            trigger_offer = "Bridal Trial @ ₹999"   # catalog default from salons.json
    elif trigger_kind == "recall_due" and "dent" in slug:
        # Look for a free add-on with cleaning
        for o in active_offers:
            if isinstance(o, dict) and o.get("type") == "free_addon":
                trigger_offer = _s(o.get("title"))
                break
        if trigger_offer:
            trigger_offer = f"Dental Cleaning @ ₹299 + {trigger_offer}"
        else:
            trigger_offer = "Dental Cleaning @ ₹299"
    elif "corporate_bulk_thali" in _s(signals.get("planning_goal")).lower():
        for o in active_offers:
            if isinstance(o, dict) and "thali" in _s(o.get("title")).lower():
                trigger_offer = _s(o.get("title"))
                break
    # Fall back to generic best_offer
    signals["context_offer"] = trigger_offer or signals["best_offer"]

    # ── CURIOUS ASK GUESS ─────────────────────────────────────────
    guess = None
    top_services = _safe_list(agg.get("top_services"))
    if top_services and isinstance(top_services[0], dict):
        guess = _s(top_services[0].get("name"))
    if not guess:
        for trend in trend_signals:
            tq = _s(trend.get("query")).lower()
            for title in active_offer_titles:
                if tq.split()[0] in title.lower():
                    guess = title
                    break
            if guess:
                break
    if not guess:
        guess = active_offer_titles[0] if active_offer_titles else "your top service"
    signals["curious_guess"] = guess
